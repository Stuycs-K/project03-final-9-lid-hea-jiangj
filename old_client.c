...
char * display_forum = "forum.txt";
char * file_name = "forum.txt";
...
int clientLogic(int server_socket, int filtered){
    ...
    input_pos = 0;
    char prompt[] = "Input a command (post, view, edit, delete, search, sort, exit): ";
    ...
    // check if the inputted command is [sort]
        ...
        printw("How would you like your post sorted?(alphabetical, recency, normal): ");
        getstr(input);
        strcat(input,"\n");
        printw("===================================================\n");
        refresh();
        ...
        if(strcmp(reply, "NO") != 0){
            ...
            printw("%s SORTED: \n===================================================\n%s===================================================\n", input, content);
            display_forum = strdup(input);
            scroll_position = -1;
            refresh();
            sleep(2);
            return 1;
        }
        else if(strcmp(reply, "OKAY") == 0){
            scroll_position = -1;
            display_forum = strdup("forum.txt");
        }
        else{
